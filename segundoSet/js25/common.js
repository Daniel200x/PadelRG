...
// Función para renderizar una categoría completa
...
// Función para renderizar una edición completa
function renderizarEdicion(edicion, categoriaKey) {
    const contenedor = document.getElementById('contenido-ediciones');
    contenedor.innerHTML = "";

    if (!categoriaKey || !edicion.categorias[categoriaKey]) {
        contenedor.innerHTML = "<p>Seleccione una categoría para ver los resultados.</p>";
        return;
    }

    const categoria = edicion.categorias[categoriaKey];
    const categoriaDiv = document.createElement("div");
    categoriaDiv.className = "categoria";

    const titulo = document.createElement("h4");
    titulo.textContent = `${categoria.nombre} - ${edicion.nombre} (${edicion.fecha})`;
